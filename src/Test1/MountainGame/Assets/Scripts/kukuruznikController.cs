...
public class kukuruznikController : MonoBehaviour
{
    ...
    private float changePlaneMoveSpeed = 0.5f;
    ...
    private void Update()
    {
        ...
        if (rotationInputY > 0 || Input.GetKey(KeyCode.Space))
        {
        ...
        source.pitch = Input.GetAxis("Vertical") > 0 || Input.GetKey(KeyCode.Space) ? Mathf.Lerp(source.pitch, 1.5f, Time.deltaTime * smoothTime) : Mathf.Lerp(source.pitch, 1.0f, Time.deltaTime * smoothTime);
        ...
        if (middleThing != null)
        {
            middleThing.localRotation = Quaternion.Euler(middleThing.localEulerAngles.x, middleThing.localEulerAngles.y, Mathf.Lerp(middleThing.localEulerAngles.z, Mathf.Clamp(middleThing.localEulerAngles.z, middleThing.localEulerAngles.z - rotationYaw * 15f, middleThing.localEulerAngles.z - rotationYaw * 15f), Time.deltaTime * smoothTime));
        }
    ...
    private void AutoBalance()
    {
        ...
        if (middleThing != null)
        {
            float mCurrentRotationY = middleThing.localEulerAngles.z;
            ...
            // Применяем измененный угол поворота
            middleThing.localRotation = Quaternion.Euler(middleThing.localEulerAngles.x, middleThing.localEulerAngles.y, Mathf.Lerp(mCurrentRotationY, mTargetRotationY, Time.deltaTime * smoothTime / 3));
        }
