...
public class PerlinTileTest : MonoBehaviour {
    ...
    void Update3D () {
        //このクラス(どちらかといえばコンストラクタ)はどこで定義したんだ?
        //Simple Noise Script から参照している?
        //このseed値でノイズを生成することにする
        SimplexNoise noise = new SimplexNoise (seed);
        //res * resのノイズマップを生成
        float[] noiseMap = new float[res * res];
        float maxVal = float.MinValue;
        ...
        for (int x = 0; x < res; x++) {
            ...
            //ノイズマップのx軸のメモリごとに
            //配列は0から始まるので、res-1をしてループごとに値が等しくなるようにしている
            float px = x / (res - 1f);
            for (int y = 0; y < res; y++) {
